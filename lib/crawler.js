...
const HUB_PATHS = ['/blog', '/blogs', '/posts', '/articles', '/news', '/category/blog', '/all-posts'];
...
async function fetchPage(url, timeoutMs = 5000) {
  ...
  try {
    const res = await fetch(url, {
      ...
      redirect: 'follow',
      headers: { 'User-Agent': 'AdSenseAuditPro/2.0 (readiness-checker)', Accept: 'text/html,application/xhtml+xml,application/xml' },
    });
    clearTimeout(timer);
    const contentType = res.headers.get('content-type') || '';
    if (!res.ok) return { ok: false, status: res.status, finalUrl: res.url, html: '' };
    const html = await res.text();
    return { ok: true, status: res.status, finalUrl: res.url, html: html.slice(0, 400000), contentType };
  } catch (err) {
    clearTimeout(timer);
...
function analyzePageContent(html, url) {
  ...
  try {
    ...
    $('a[href]').each((_, el) => {
      ...
      if (href.startsWith('http') && !href.includes(host)) outbound.push(href);
    });
  } catch (_) {}
  ...
  if (h2Count >= 2) q += 8;
  if (imgCount >= 1) q += 5;
  if (maxDensity > 0.08) q -= 20;
  if (riskList.length) q -= 30;
  if (outbound.length > 30) q -= 10;
...
function collectLinksFromHtml(html, base, seen) {
  const $ = cheerio.load(html);
  ...
  $('a[href]').each((_, el) => {
    ...
    try {
      ...
      if (/\/(privacy|about|contact|terms|login|cart|tag|category|author|wp-admin|feed)/i.test(path)) return;
      if (/\.(jpg|png|gif|css|js|pdf|zip|xml|json)$/i.test(path)) return;
...
async function collectFromSitemap(base, seen, limit) {
  const links = [];
  for (const path of ['/sitemap.xml', '/sitemap_index.xml', '/post-sitemap.xml', '/page-sitemap.xml']) {
    if (links.length >= limit) break;
    const page = await fetchPage(base + path, 4000);
    if (!page.ok || !page.html) continue;
    const locs = [...page.html.matchAll(/<loc>\s*([^<\s]+)\s*<\/loc>/gi)].map((m) => m[1].trim());
    for (const loc of locs) {
      if (links.length >= limit) break;
      try {
        const u = new URL(loc);
        if (u.origin !== base && u.origin !== new URL(base).origin) continue;
        if (loc.endsWith('.xml')) continue;
        const full = u.origin + u.pathname.replace(/\/$/, '');
        if (seen.has(full) || full === base || full === base + '/') continue;
        seen.add(full);
        links.push({ url: full, score: 2 });
      } catch (_) {}
    }
  }
  return links;
...
async function gatherCandidates(base, homeHtml, maxSamples) {
  const seen = new Set();
  let links = collectLinksFromHtml(homeHtml, base, seen);

  // Hub pages for more internal URLs
  const hubFetches = await Promise.all(
    HUB_PATHS.map((p) => fetchPage(base + p, 3500))
  );
  for (const page of hubFetches) {
    if (page.ok && page.html) {
      links = links.concat(collectLinksFromHtml(page.html, base, seen));
    }
  }

  // Sitemap for large sites
  if (links.length < maxSamples) {
    const fromMap = await collectFromSitemap(base, seen, maxSamples - links.length);
    links = links.concat(fromMap);
  }

  links.sort((a, b) => b.score - a.score);
  return links.slice(0, maxSamples);
}
...
async function findLegalPage(base, paths, keywords) {
  for (const path of paths) {
    ...
    if (matches >= 1 || page.html.length > 800) {
      return { found: true, url: page.finalUrl || base + path, mentionsCookies: /cookie/.test(lower), mentionsAdsense: /adsense|doubleclick|google ads/.test(lower) };
    }
  }
  return { found: false };
}

/** Free up to 20, Pro up to 200 — batched parallel fetches */
export async function crawlSite(inputUrl, options = {}) {
  const maxSamples = Math.min(Math.max(Number(options.maxSamples) || 20, 1), 200);
  const base = normalizeBase(inputUrl);
  const home = await fetchPage(base + '/', 8000);
  if (!home.ok) {
    return { success: false, error: home.error || `Could not reach site (status ${home.status})`, hostname: new URL(base).hostname };
  }
  ...
  const candidates = await gatherCandidates(base, home.html, maxSamples);
  const concurrency = maxSamples > 50 ? 12 : 8;
  const sampleResults = await mapPool(candidates, concurrency, async (c) => {
    const page = await fetchPage(c.url, 4000);
    if (!page.ok || !page.html) return null;
    if ((page.contentType || '').includes('xml')) return null;
    return analyzePageContent(page.html, page.finalUrl || c.url);
  });
  const sampled = sampleResults.filter(Boolean);
  ...
  const checks = [
    { id: 'https', label: 'HTTPS / SSL active', critical: true, pass: usesHttps, detail: usesHttps ? `Loads over HTTPS` : 'Did not confirm HTTPS', fix: 'Enable SSL and force HTTPS.' },
    { id: 'privacy', label: 'Privacy Policy page', critical: true, pass: privacy.found, detail: privacy.found ? 'Found' : 'Not found', recommendedPage: '/privacy-policy', fix: 'Create Privacy Policy with cookies + AdSense disclosure.', sampleText: privacy.found ? null : 'Privacy Policy\n\nWe use cookies. Google uses DoubleClick.' },
    { id: 'about', label: 'About / Author page (E-E-A-T)', critical: true, pass: about.found, detail: about.found ? 'Found' : 'Not found', recommendedPage: '/about', fix: 'Add About with real author identity.', sampleText: about.found ? null : 'About Us\n\nI am [Name], founder of [Site].' },
    ...
    { id: 'content-quality', label: 'Overall content quality', critical: true, pass: avgQuality >= 50 && lowCount <= highCount, detail: `Avg quality ${avgQuality}/100 · ${allPages.length} pages (target up to ${maxSamples} posts) · ${highCount} high · ${lowCount} low`, fix: 'Publish longer original articles (800+ words).' },
    { id: 'policy-risk', label: 'Publisher policy risk (nulled/adult/etc.)', critical: true, pass: siteRisks.length === 0, detail: siteRisks.length ? `Risk: ${siteRisks.join(', ')} on ${riskPages.length} page(s)` : 'No piracy/nulled/adult/gambling/malware signals', fix: 'Remove prohibited content.' },
    { id: 'thin-posts', label: 'Sampled posts not thin', critical: false, pass: sampled.length === 0 || sampled.filter((p) => p.wordCount < 250).length <= sampled.length / 2, detail: sampled.length ? `${sampled.filter((p) => p.wordCount < 250).length}/${sampled.length} under 250 words` : 'Few post links found to sample', fix: 'Aim for 800–1500+ words per article.' },
    ...
    { id: 'schema', label: 'Structured data', critical: false, pass: a.hasStructuredData, detail: a.hasStructuredData ? 'Schema found' : 'None', fix: 'Optional Article schema.' },
  ];
