...
async function fetchPage(url, timeoutMs = 5000) {
  ...
  try {
    ...
    return { ok: true, status: res.status, finalUrl: res.url, html: html.slice(0, 400000) };
  } catch (err) {
    clearTimeout(timer);
...
/** maxSamples: Free ~3, Pro ~10 (Vercel Hobby 10s limit with parallel fetches) */
export async function crawlSite(inputUrl, options = {}) {
  const maxSamples = Math.min(Math.max(Number(options.maxSamples) || 5, 1), 10);
  const base = normalizeBase(inputUrl);
  const home = await fetchPage(base + '/', 7000);
  if (!home.ok) {
  ...
  const candidates = extractArticleLinks(home.html, base).slice(0, maxSamples);
  const sampleResults = await Promise.all(
    candidates.map(async (c) => {
      const page = await fetchPage(c.url, 4500);
  ...
  const checks = [
    ...
    { id: 'content-quality', label: 'Overall content quality', critical: true, pass: avgQuality >= 50 && lowCount <= highCount, detail: `Avg quality ${avgQuality}/100 · ${allPages.length} pages (max ${maxSamples} posts) · ${highCount} high · ${lowCount} low`, fix: 'Publish longer original articles (800+ words).' },
