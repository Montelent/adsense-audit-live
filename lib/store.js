...
function getStore() {
  if (!global[globalKey]) global[globalKey] = defaultStore();
  const s = global[globalKey];
...
function getAds() { return { ...getStore().ads }; }
function updateAds(data) {
  const store = getStore();
  store.ads = { ...store.ads, ...data };
  return store.ads;
}
function getScripts() { return { ...getStore().scripts }; }
...
module.exports = {
  ...
  getSettings, updateSettings, getAds, updateAds, getScripts, updateScripts,
  listUsers, findUserByEmail, updateUserPassword, updateUser, recordAudit, getStats, getStore,
};
