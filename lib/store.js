...
export async function updateUser(userId, data) {
  if (hasDatabase()) {
    ...
    const password =
      data.password && data.password.length >= 6 ? data.password : existing.password;
    await db`
      UPDATE users SET
        name = ${name},
        email = ${email},
      ...
      WHERE id = ${userId}
    `;
    const { password: _p, resetToken, resetExpires, ...out } = {
      ...existing,
      name,
      email,
      role,
...
export async function createBlog(data) {
  ...
  if (hasDatabase()) {
    const db = await ensureSchema();
    await db`
      INSERT INTO blogs (id, title, slug, excerpt, content, author, published, created_at, updated_at)
      VALUES (${blog.id}, ${blog.title}, ${blog.slug}, ${blog.excerpt}, ${blog.content}, ${blog.author}, ${blog.published}, NOW(), NOW())
    `;
    return blog;
  }
...
// ——— Settings / plan / payments / mail via KV + memory ———
...
export async function getSettingsAsync() {
  if (hasDatabase()) {
    ...
    if (v) {
      getStore().settings = { ...getStore().settings, ...v };
      return { ...getStore().settings };
    }
  }
  return getSettings();
...
export async function getPlanAsync() {
  if (hasDatabase()) {
    const v = await kvGet('plan', null);
    if (v) {
      getStore().plan = { ...getStore().plan, ...v };
    }
  }
  return getPlan();
}
...
export async function getPaymentsAsync() {
  if (hasDatabase()) {
    const v = await kvGet('payments', null);
    if (v) getStore().payments = { ...getStore().payments, ...v };
  }
  return getPayments();
}
...
export async function getMailSettingsAsync() {
  if (hasDatabase()) {
    const v = await kvGet('mail', null);
    if (v) getStore().mail = { ...getStore().mail, ...v };
  }
...
export async function getAdsAsync() {
  if (hasDatabase()) {
    const v = await kvGet('ads', null);
    if (v) getStore().ads = { ...getStore().ads, ...v };
  }
  return getAds();
}
...
export function getScripts() {
  return { ...getStore().scripts };
}
export async function getScriptsAsync() {
  if (hasDatabase()) {
    const v = await kvGet('scripts', null);
    if (v) getStore().scripts = { ...getStore().scripts, ...v };
  }
  return getScripts();
}
...
export async function addPaymentRequest(req) {
  ...
  if (hasDatabase()) {
    const db = await ensureSchema();
    await db`
      INSERT INTO payment_requests (id, data, status, created_at)
      VALUES (${item.id}, ${JSON.stringify(item)}::jsonb, ${item.status}, NOW())
    `;
    return item;
  }
  const store = getStore();
...
export async function updatePaymentRequest(id, data) {
  if (hasDatabase()) {
    ...
    await db`
      ...
      SET data = ${JSON.stringify(next)}::jsonb, status = ${next.status || 'pending'}
      WHERE id = ${id}
    `;
    return next;
  }
