import { getPayments, updateUser, addPaymentRequest, updatePaymentRequest } from './store.js';
...
/** Activate Pro after successful auto payment */
export function activateProFromPayment({ userId, email, method, reference, amount }) {
  let uid = userId;
  if (!uid && email) {
    const { findUserByEmail } = require('./store.js');
  ...
  updatePaymentRequest(item.id, { status: 'approved', auto: true, reference });
